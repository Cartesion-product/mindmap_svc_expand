...
async def generate_sync(
    ...
    try:
        ...
        audio_paths = config.fixed_prompt_audio_paths
        prompt_text_list = config.fixed_prompt_texts


        # 验证对话格式
        is_valid, error_msg = validate_dialogue_format(dialogue_text, len(audio_paths))
        if not is_valid:
            raise HTTPException(status_code=400, detail=error_msg)

        # 保存上传的文件
        # audio_paths = []
        # for i, file in enumerate(audio_pathss):
